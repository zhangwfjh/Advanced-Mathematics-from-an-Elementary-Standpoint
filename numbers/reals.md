...
我们对于比例数和实数的构造都是通过商集实现的，但是通常我们使用时，尤其在计算机中，习惯于用十进小数表示。两种表示实际上是等价的：任意给定一个比例数$$\frac pq$$，可以通过除法化作十进小数$$p \div q$$；给定一个十进小数$$\pm b_0.b_1b_2\cdots$$，也可以构造比例数数列$$\{\pm(b_0+\frac{b_1\cdots b_n}{10^n})\}_0^\infty$$。
