...
其中：$$x \sim y$$表示$$(x,y)\in R\subset S\times S$$。例如，整数相等$$=$$是一个等价关系，自然数同余$$\equiv$$也是等价关系，但整数比较$$\lt$$或者集合包含$$\subset$$不是等价关系。
...
为了比较大小，比例数还可以定义序。对于任意两个比例数$$\frac pq,\frac rs(q,s \gt 0)$$，定义$$\frac pq \lt \frac rs$$当且仅当$$ps \lt qr$$。读者可以证明该定义也是良好的。
