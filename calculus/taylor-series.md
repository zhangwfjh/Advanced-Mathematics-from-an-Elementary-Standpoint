...
注意到$$f'(x)=\dfrac{df}{dx}=\dfrac{d}{dx}f(x)$$，不妨认为$$D:=\dfrac{d}{dx}$$是一个关于函数$$f(x)$$的函数，称作**微分算子**，即$$Df:=f'$$。则利用麦克劳林级数有$$f(a)=\sum_{n=0}^\infty\dfrac{f^{(n)}(0)}{n!}a^n=\sum_{n=0}^\infty\dfrac{(aD)^n}{n!}f(0)=e^{aD}f(0)$$。类似的，根据泰勒级数还有$$f(x)=e^{aD}f(x-a)$$。因此，我们认为$$S_a:=e^{aD}$$是是函数$$f(x)$$的**平移算子**，即对任意$$a \in \mathbb R$$，$$S_a$$将函数$$f(x)$$平移至$$f(x+a)=S_af(x)$$。泰勒展开便是函数的无穷小平移，泰勒级数是将实函数$$f$$扩展到超实数域$$^*f$$。