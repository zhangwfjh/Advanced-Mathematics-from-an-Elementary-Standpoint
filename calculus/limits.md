...
极限还有另一种解释，如果任意精度的“尺子”最终无法测量$$L$$与$$p$$附近函数值$$f(x)$$之间的距离，那么$$f(x)$$在$$x=p$$处的极限为$$L$$。用数学语言表达为：$$\forall \epsilon > 0, \exists \delta > 0, \forall x \in (p-\delta, p+\delta): |f(x)-L| < \epsilon$$。读者可以将此与柯西列极限作比较。
